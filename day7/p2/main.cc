...
void get_vector(int n,string s,vector<long long>& sol){
    string ss = s;
    int pos=0,aux=0;
    while(aux != -1){
...
int get_power(long long sol){
    int digit = 0;
    long long number;

    for(number=sol; number>0; digit++) {
        number /= 10;
    }
    return digit;
...
bool get_sol(const vector<long long>& sol, int idx, long long  &n, long long total){
    ...
    for(int i = 0; i<3; i++){
        ...
        if(idx == 0) value = sol[idx];
            ...
            int power = get_power(sol[idx]);
            value = total*pow(10.0,float(power)) + sol[idx];
            //cout << power << endl;
        }
